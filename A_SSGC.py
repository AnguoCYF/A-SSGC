...
from adaptive_threshold_strategy import adaptive_threshold_sparsify
...
def build_dgl_graph(features, labels, method, param, fuse_knn=10, fuse_euc=5, fuse_mah=5, fuse_cos=95):
    ...
    if method == 'cosine':
        ...
        adjacency_matrix, _ = generate_euclidean_threshold_adjacency_matrix(features, param)
    elif method == 'mst':
        ...
        adj_euc, euclidean_distances = generate_euclidean_threshold_adjacency_matrix(features, fuse_euc)
        adj_mst = generate_mst_adjacency_matrix(features, euclidean_distances)

        adjacency_matrix = fuse_adjacency_matrices_similarity_based([adj_knn, adj_mst, adj_cos, adj_mah, adj_euc], param[0], param[1])

        del adj_knn, adj_cos, adj_mah, adj_euc, adj_mst
        gc.collect()
...
def similarity_measure(matrix1, matrix2):
    ...
    # 使用均值距离来标准化距离
    mean_distance = (np.linalg.norm(matrix1_dense, 'fro') + np.linalg.norm(matrix2_dense, 'fro')) / 2
    normalized_distance = distance / mean_distance
...
def fuse_adjacency_matrices_similarity_based(matrix_list, base_threshold=0.5, degree_factor=0.005):
    """
    融合邻接矩阵的相似度基准方法
    """
    n_matrices = len(matrix_list)
    ...
    threshold_fused_matrix = adaptive_threshold_sparsify(fused_matrix, base_threshold, degree_factor)
